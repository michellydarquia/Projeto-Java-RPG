...
public class Cajado extends Item {
    ...
    public Cajado() {
        super("Cajado", "+30 ATAQUE MÁGICO", "EQUIPAR");
    }

    @Override
    public BufferedImage getImagem() {
        try {
            imagem = ImageIO.read(Objects.requireNonNull(getClass().getResourceAsStream("/itens/Cajado.png")));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return imagem;
    ...
    public int getBonusAtaque(Personagem personagem) {
        return 15; // Cajado não dá bônus de ataque físico
    }

    @Override
    public int getBonusDefesa(Personagem personagem) {
        return 0; // Cajado não dá bônus de defesa
    }
}
